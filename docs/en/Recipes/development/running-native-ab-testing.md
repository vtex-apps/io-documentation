...
2. `What's the amount of time respecting the restriction?`. This is the amount of time (in hours) that the traffic proportion stated in the previous question remains constant. After this period, the A/B testing system begins to automatically update traffic proportions by measuring the performance of each workspace and sending more traffic to the best-performing one. There are two possible answers to this question:
