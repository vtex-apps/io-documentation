...
Ao usar o VTEX IO, toda interação com uma conta ocorre em um [***workspace***](*link*).

Ao fazer *login* em uma loja, você estará automaticamente no *workspace* master dela, ou seja, na versão disponível para o usuário final. Por isso, lembre-se de que sempre que você quiser testar uma nova configuração, o seu próprio *workspace* de desenvolvimento deverá ser criado, usando o comando `vtex use`.
...
Considere que `workspace` é o *workspace* que você acabou de criar (como `nomeexemplo`) e `conta` é o nome da conta em que você está trabalhando.
